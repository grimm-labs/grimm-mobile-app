...
import { convertBitcoinToFiat, convertSatsToBtc, getFiatCurrency, mergeAndSortSparkTransactions } from '@/lib';
import { AppContext, useSpark } from '@/lib/context';
...
export default function LnWalletDetails() {
  ...
  const { balance, payments } = useSpark();
  const selectedFiatCurrency = getFiatCurrency(selectedCountry);
  const [transactions, setTransactions] = React.useState<UnifiedTransaction[]>([]);

  useEffect(() => {
    const unified = mergeAndSortSparkTransactions(payments, []);
